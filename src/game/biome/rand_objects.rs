...
/// Spawns a specified number of rocks randomly along a road segment,
/// ensuring a minimum distance between them.
pub fn spawn_rocks_on_road(
    ...
    let rock_handle: Handle<Scene> = asset_server.load("3dmodels/rock/scene.gltf#Scene0");
    let mut rng = thread_rng();
    let mut placed_rock_positions: Vec<Vec2> = Vec::with_capacity(num_rocks);

    let half_distance = road_distance / 2.0;
    // Keep rocks slightly away from the absolute edges
    ...
    for _ in 0..num_rocks {
        ...
        loop {
            if attempts >= MAX_PLACEMENT_ATTEMPTS {
                ...
                eprintln!("Warning: Could not place a rock after {} attempts. Skipping.", MAX_PLACEMENT_ATTEMPTS);
                break; 
            }
            attempts += 1;
            ...
            for &placed_pos in &placed_rock_positions {
                // if rocks are too close, try again
            ...
            if !too_close {
                placed_rock_positions.push(current_pos_2d);
                ...
                commands.entity(road_parent_id).with_children(|parent| {
                    parent.spawn((
                        SceneRoot(rock_handle.clone()),
                        Transform {
                            translation: Vec3::new(local_x, local_y, local_z),
                            rotation: Quat::IDENTITY, 
                            scale: Vec3::splat(rng.gen_range(0.01..0.02)), // Random size
                        },
                        GlobalTransform::default(),
                        Visibility::default(),
                    ));
                });
                break; // Go to the next rock
            }
        }
    }
}